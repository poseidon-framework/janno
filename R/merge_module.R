merge_module <- function(input_file, output_directory, log_directory) {
  ...
  merge_concat_janno_files(list_of_packages, output_directory)
  ...
  plink_merge(plink_merge_file, plink_order_file, output_directory, "poseidon2_merged", log_directory)
...
merge_create_order_file_from_fam_files <- function(list_of_packages, log_directory) {
  ...
  plink_order_file <- file.path(output_directory, "poseidon2_merge_plink_order_file.txt")
  readr::write_tsv(concat_first_two_columns, path = plink_order_file, col_names = FALSE)
  return(plink_order_file)
}

merge_concat_janno_files <- function(list_of_packages, output_directory) {
  ...
  readr::write_tsv(new_janno, path = file.path(output_directory, "hu.janno"))
}
